...
public class TextFunction {
  ...
  private FunctionResolver ltrim() {
    return define(BuiltinFunctionName.LTRIM.getName(),
            impl(nullMissingHandling((v) -> new ExprStringValue(stripLeading(v.stringValue().getBytes()))),
                    STRING, STRING));
  }
  ...
  private FunctionResolver rtrim() {
    return define(BuiltinFunctionName.RTRIM.getName(),
            impl(nullMissingHandling((v) -> new ExprStringValue(stripTrailing(v.stringValue().getBytes()))),
                    STRING, STRING));
  }
