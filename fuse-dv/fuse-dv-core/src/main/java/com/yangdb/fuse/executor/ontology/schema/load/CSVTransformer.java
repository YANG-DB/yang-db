...
public class CSVTransformer implements DataTransformer<DataTransformerContext, CSVTransformer.CsvElement> {
    ...
    @Inject
    public CSVTransformer(Config config, OntologyProvider ontology, IndexProviderIfc indexProvider, RawSchema schema, IdGeneratorDriver<Range> idGenerator, Client client) {
    ...
    private DocumentBuilder translate(DataTransformerContext context, EntityType entityType, Map<String, String> node) {
        try {
            ...
            return new DocumentBuilder(element, node.get(ID), entity.getType(), Optional.empty());
    ...
    private void populatePropertyFields(DataTransformerContext context, Map<String, String> node, Relation relation, ObjectNode element, String direction) {
        ...
        switch (direction) {
            case "out":
                element.put(ENTITY_A, populateSide(ENTITY_A, context, node.get(SOURCE),node.get("source.type"), relation,node));
                //populate redundant fields B
                element.put(ENTITY_B, populateSide(ENTITY_B, context, node.get(TARGET),node.get("target.type"), relation,node));
                break;
            case "in":
                element.put(ENTITY_B, populateSide(ENTITY_A, context, node.get(SOURCE),node.get("source.type"), relation,node));
                //populate redundant fields B
                element.put(ENTITY_A, populateSide(ENTITY_B, context, node.get(TARGET),node.get("target.type"), relation,node));
                break;
        }
      }
    ...
    private ObjectNode populateSide(String side, DataTransformerContext context, String sideId,String sideType, Relation relation,Map<String, String> node) {
        String sideName = side.equals(ENTITY_A) ? SOURCE :  ( side.equals(ENTITY_B) ? TARGET : "unknown");
        ...
        redundant.forEach(r -> populateRedundantField(r, node, sideName,entitySide));
        return entitySide;
    }
