...
import com.kayhut.fuse.services.engine2.SmartEpbM2ComplexQueriesTestSuite;
...
public class ComplexQueriesTest {
    ...
    public static void setup() throws Exception {
        setup(SmartEpbM2ComplexQueriesTestSuite.elasticEmbeddedNode.getClient(), true);
    }

    @AfterClass
    public static void cleanup() throws Exception {
        cleanup(SmartEpbM2ComplexQueriesTestSuite.elasticEmbeddedNode.getClient());
    }
    ...
    @Test
    public void testDragonFireDragonPathConcrete() throws IOException, InterruptedException {
        Query query = Query.Builder.instance().withName(NAME.name).withOnt($ont.name()).withElements(Arrays.asList(
                new Start(0, 1),
                new EConcrete(1, "A", $ont.eType$(DRAGON.name), "Dragon_4", "Dragon_0", singletonList(NAME.type), 2, 0),
                ...
                new EConcrete(5, "C", $ont.eType$(DRAGON.name), "Dragon_9", "Dragon_1", singletonList(NAME.type), 0, 0)
        )).build();
