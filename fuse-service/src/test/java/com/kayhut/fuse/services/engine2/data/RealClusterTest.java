...
public class RealClusterTest {
    ...
    /**
     * Plan: {plan=[11:12:9:10:6:8:4:5:1:3], cost=EntityOp(Asg(ETyped(11))):EntityFilterOp(Asg(EPropGroup(12))):RelationOp(Asg(Rel(9))):RelationFilterOp(Asg(RelPropGroup(10))):EntityOp(Asg(ETyped(6))):EntityFilterOp(Asg(EPropGroup(8))):RelationOp(Asg(Rel(4))):RelationFilterOp(Asg(RelPropGroup(5))):EntityOp(Asg(ETyped(1))):EntityFilterOp(Asg(EPropGroup(3))) >> DoubleCost{cost=1000000.0}}
    ...
    public void test8() throws IOException, InterruptedException, ParseException {
        ...
        Query query = Query.Builder.instance().withName("Q1").withOnt($ont.name()).withElements(Arrays.asList(
                new Start(0, 1),
                new ETyped(1, "A", DRAGON.type, Collections.emptyList(), 2, 0),
                new Rel(2, OWN.getrType(), Rel.Direction.L, null, 3, 0),
                new ETyped(3, "B", PERSON.type, Collections.emptyList(), 0, 0)))
                .build();
