        ...
        for(int i = 0 ; i < numDragons ; i++) {
            for (int j = 0; j < i; j++) {
                ...
                Entity entityB = Entity.Builder.instance()
                        ...
                        .withEType($ont.eType$(DRAGON.name))
                        .withProperties(Arrays.asList(
                                new com.yangdb.fuse.model.results.Property(NAME.type, "raw", DRAGON.name + j),
                                new com.yangdb.fuse.model.results.Property(BIRTH_DATE.type, "raw", sdf.format(new Date(birthDateValueFunction.apply(j))))))
                        .build();
