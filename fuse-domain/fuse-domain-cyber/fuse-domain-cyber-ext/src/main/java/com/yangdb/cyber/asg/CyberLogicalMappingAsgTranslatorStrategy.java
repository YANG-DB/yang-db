...
/**
 * replace logical graph query (fields within the Entity) into schema graph query (RDF structure - fields as separate nodes)
 * The mapping of the logical ontology to the schema ontology is currently mapped in this code - should be auto generated by UI mapper
 * or created by some json based mapping config file
 */
public class CyberLogicalMappingAsgTranslatorStrategy implements AsgStrategy {
    ...
    public CyberLogicalMappingAsgTranslatorStrategy(OntologyProvider ontologyProvider, OntologyMappingProvider mappingProvider) {
    ...
    @Override
    /**
     * Here we will take the logical ontology entities and according to the indexProvider mapping schema (schemaProviderFactory)
     * for each entity / relation :
     *          find the element's (entity/rel) physical index partition name in the logical schema and match it to the index provider in the physical schema
     *              - once found derive the element's (entity/rel) name from the physical schema provider and according to that name find the element
     *                in the schema ontology
     */
    public void apply(AsgQuery query, AsgStrategyContext context) {
        ...
        Ontology schema = ontologyProvider.get(CYBER)
                .orElseThrow(() -> new FuseError.FuseErrorException(new FuseError("No Ontology found ", "No Ontology found with name [" + CYBER + "]")));

        String labelFieldName = schemaProviderFactory.get(schema).getLabelFieldName().orElseGet(() -> "value");
        Ontology logicalOntology = this.ontologyProvider.get(query.getOnt())
                .orElseThrow(() -> new FuseError.FuseErrorException(new FuseError("No Ontology found ", "No Ontology found with name [" + query.getOnt() + "]")));
        Ontology.Accessor logicalOntAccessor = new Ontology.Accessor(logicalOntology);
        Ontology.Accessor schemaAccessor = new Ontology.Accessor(schema);

        AtomicInteger counter = new AtomicInteger(AsgQueryUtil.max(query));

        //break logical properties constaints to knowledge entity constraint (Evalue)
        translateLogicalProperties(labelFieldName, query, counter, logicalOntAccessor, schemaAccessor);

        //break logical entity to knowledge entity with constraint on Category (label)
        translateLogicalEntity(labelFieldName, query, counter, logicalOntAccessor, schemaAccessor);

        //break logical relations to knowledge entity with constraint on Category (label)
        translateLogicalRelation(labelFieldName, query, counter, logicalOntAccessor, schemaAccessor);
    ...
    private void translateLogicalProperties(String labelFieldName, AsgQuery query, AtomicInteger counter, Ontology.Accessor logicalOntAccessor, Ontology.Accessor knowledgeOntAccessor) {
        ...
        AsgQueryUtil.getEprops(query)
                ...
                .forEach(eProp -> {
                    ...
                    //replace Evalue
                    replace(counter, logicalOntAccessor, eProp, asgEprop, AsgQueryUtil.pathToAncestor(asgEprop.get(), ETyped.class), EVALUE);
                });
