...
public class QueryDriverTest extends BaseModuleInjectionTest {
    ...
    public void setupData(Client client) throws ParseException, JsonProcessingException {
        ...
        Assert.assertEquals(7, commit(ctx, INDEX, f1));
    }
    ...
    public void testCreate() {
        ...
        final Optional<QueryResourceInfo> info = driver.create(new QueryMetadata("1", "q", 1000), query);
        Assert.assertTrue(info.isPresent());
        Assert.assertTrue(info.get().getResourceUrl().endsWith("/fuse/query"));
