...
public class Elapsed {
    @Plugin(name = "elapsed", category = "Converter")
    @ConverterKeys({"elapsed"})
    public static class Converter extends LogEventPatternConverter {
        public static Converter newInstance(final String[] options) {
            ...
            return new Converter("elapsed", "elapsed", timeUnit);
        }
        ...
        public void format(LogEvent logEvent, StringBuilder sb) {
            String start = logEvent.getContextData().getValue(Epoch.StartNanos.key);
            if (start == null) {
                sb.append("0");
            } else {
