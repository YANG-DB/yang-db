...
public class SmartEpbShortPathTests {
    ...
    public void testPathSelectionNoConditions() {
        ...
        Iterator<PlanWithCost<Plan, Cost>> iterator = first.getCost().getPlanStepCosts().iterator();
        PlanWithCost<Plan, Cost> op = iterator.next();
    ...
    public void testBackUpdateOfEstimates() {
        ...
        PlanWithCost<Plan, Cost> opWithCost = plan.getCost().getPlanStepCosts().iterator().next();
        Assert.assertEquals(3, ((CountEstimatesCost)opWithCost.getCost()).getCountEstimates().size());
        Iterator<Double> iterator = ((CountEstimatesCost)opWithCost.getCost()).getCountEstimates().iterator();
