...
public class ChainedPlanOpTraversalTranslator implements PlanTraversalTranslator {
    ...
    public GraphTraversal<?, ?> translate(Plan plan, TranslationContext context) {
        ...
        for (int planOpIndex = this.startFrom; planOpIndex < plan.getOps().size(); planOpIndex++) {
            traversal = this.translationStrategy.translate(traversal, plan, plan.getOps().get(planOpIndex), context);
        }
