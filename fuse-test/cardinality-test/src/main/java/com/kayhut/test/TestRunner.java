...
public class TestRunner {
    public static void main(String[] args) throws CardinalityMergeException, IOException {
        ...
        try(CSVWriter writer = new CSVWriter(Files.newBufferedWriter(Paths.get("intersections.csv").toAbsolutePath()))) {
            ...
            for (int p = 4; p < 16; p++) {
                for (int bucketWidth = 10; bucketWidth <= 50; bucketWidth += 10) {
                    ...
                    for (double overlap = 0; overlap < 1; overlap += 0.25) {
                        //for (int k = 256; k <= 1024; k += 256) {
                            Random random = new Random(1234);
                            int sp = p;
                            int numInstances = 3000000;
                            //Test test = new MultiModalGaussianIntersectionMinHashTest(random, p, sp, numInstances, bucketWidth, k, overlap);
                            ...
                            //Test test = new MultiModalGaussianIntersectionMinHashZeroTest(random, p, sp, numInstances,bucketWidth, 100,overlap);
