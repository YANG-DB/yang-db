...
public class TestRunner {
    public static void main(String[] args) throws CardinalityMergeException, IOException {
        ...
        try(CSVWriter writer = new CSVWriter(Files.newBufferedWriter(Paths.get("intersections-fields.csv").toAbsolutePath()))) {

            String[] header = new String[]{"p", "bucket size", "error ratio", "absolute error", "absolute zero error", "average intersection size", "overlap ratio", "num fields"};
            ...
            for (int p = 4; p < 16; p++) {
                for (int bucketWidth = 10; bucketWidth <= 50; bucketWidth += 10) {
                    ...
                    //for (double overlap = 0; overlap < 1; overlap += 0.25) {
                        ...
                        Test test = new MultiModalGaussianIntersectionTest(random, p, sp, numInstances, bucketWidth, overlap,f);
