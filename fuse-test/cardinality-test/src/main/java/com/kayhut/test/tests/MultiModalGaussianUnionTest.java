...
public class MultiModalGaussianUnionTest implements Test {



    public MultiModalGaussianUnionTest(Random random, int p, int sp, int numInstances) {
    ...
    public TestResults run() throws CardinalityMergeException {
        ...
        for (HLLHistogramBucket<Integer, Integer> valueBucket1 : fieldValueHistogram.getBuckets()) {
            if(valueBucket1.getObjects().size() == 0) {
                continue;
            }

            for(HLLHistogramBucket<Integer, Integer> valueBucket2 : fieldValueHistogram.getBuckets()){
                if(valueBucket2.getObjects().size() == 0 || valueBucket1.equals(valueBucket2)) {
                    continue;
                }
