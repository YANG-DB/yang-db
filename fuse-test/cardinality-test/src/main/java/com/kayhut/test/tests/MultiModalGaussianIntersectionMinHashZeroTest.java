...
public class MultiModalGaussianIntersectionMinHashZeroTest extends MultiModalGaussianIntersectionBase{

    public MultiModalGaussianIntersectionMinHashZeroTest(Random random, int p, int sp, int numInstances, int bucketWidth, int k, double overlapRatio) {
    ...
    public MultiModalGaussianIntersectionMinHashZeroTest(Random random, int p, int sp, int numInstances, int bucketWidth, int k, double overlapRatio, int numFields) {
    ...
    public TestResults run() throws CardinalityMergeException {
        ...
        for (HLLMinHashHistogramBucket<Integer, Integer> fieldBucket : fieldHistogram.getBuckets()) {
            for (HLLMinHashHistogramBucket<Integer, Integer> valueBucket : fieldValueHistogram.getBuckets()) {
                ...
                double estimatedJaccard = fieldBucket.getMinHash().estimateJaccard(valueBucket.getMinHash());
                long intersect;
                if(estimatedJaccard == 0){
                    intersect = 0;
                }else{
                    intersect = HLLUtils.intersect(fieldBucket.getHll(), valueBucket.getHll());
                }
